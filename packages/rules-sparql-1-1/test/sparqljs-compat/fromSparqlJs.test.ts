...
import type {
  ...
  Term,
  Update,
} from '../../lib/Sparql11types.js';
import type * as SparqlJs from '../../lib/sparqljs-compat/sparqljsTypes.js';
...
/**
 * Sparqljs' published type declarations (`@types/sparqljs`) are free to drift from the structural
 * mirror this package maintains on purpose (see sparqljsTypes.ts, and the file header of fromSparqlJs.ts
 * for why it is not depended on directly). A real parse result is always structurally compatible with
 * our mirror at runtime, so the cast is confined to this single boundary function.
 */
function parseRaw(query: string): SparqlJs.SparqlQuery {
  return <SparqlJs.SparqlQuery> <unknown> sparqlJsParser.parse(query);
}
...
describe('fromSparqlJs', () => {
  describe('termFromSparqlJs', () => {
    ...
    describe('infers termType when it is missing (e.g. lost to a non-enumerable getter, see file header)', () => {
      ...
      it('infers a Literal from the presence of datatype/language/direction keys', ({ expect }) => {
      ...
      it('still throws on a quoted-triple-shaped (subject/predicate/object) value', ({ expect }) => {
        ...
        expect(() => termFromSparqlJs(quad)).toThrow(/RDF-star/u);
      });
      ...
      it('recognizes a termType-less quoted triple reached via an expression position too', ({ expect }) => {
        ...
        expect(() => expressionFromSparqlJs(quad)).toThrow(/RDF-star/u);
      });
    ...
    it('throws on RDF-star / SPARQL-star quoted triples', ({ expect }) => {
      const ast = <SparqlJs.SelectQuery> <unknown> sparqlStarParser.parse('SELECT * WHERE { << ?s ?p ?o >> ?p2 ?o2 }');
      const bgp = firstRawBgp(ast.where!);
      expect(() => termFromSparqlJs(bgp.triples[0].subject)).toThrow(/RDF-star/u);
    });
  });
