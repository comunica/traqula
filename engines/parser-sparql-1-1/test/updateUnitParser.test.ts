...
import { updateParserBuilder } from '../lib';
...
describe('a SPARQL 1.1 objectlist parser', () => {
  ...
  function parse(query: string, context: Partial<SparqlContext>): Update | ContextDefinition[] {
    const parser = updateParserBuilder.consumeToParser({
...
# Anti pear rally
    ...
    pears
    """@en-GB ;
    xsd:string "normal mango"^^xsd:string, ?a ;
}
Insert
{
    xsd:string xsd:integer "\\"apple\\"" ;
                xsd:string "normal mango"@en-GB , _:blank, ?a .
}
Using nameD :graph 
WhErE {
    ?s ?p ?o
