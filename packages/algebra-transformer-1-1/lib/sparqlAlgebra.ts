...
// 18.2.1
function inScopeVariables(thingy: SparqlQuery | Pattern | PropertyPath | RDF.Term): Record<string, RDF.Variable> {
  ...
  if (isTriple(thingy)) {
    ...
    for (const v of thingy.variables) {
      if (isVariable(v)) {
        ...
        Object.assign(inScope, inScopeVariables((<VariableExpression> v).variable));
      }
    }
      ...
      if (thingy.group) {
        ...
        for (const g of thingy.group) {
          if ((<VariableExpression> g).variable) {
            Object.assign(inScope, inScopeVariables((<VariableExpression> g).variable));
          }
        }
      }
      if (thingy.values) {
...
function translateGraphPattern(thingy: Pattern): Algebra.Operation {
  ...
  if (thingy.type === 'group') {
    ...
    for (const pattern of thingy.patterns) {
    ...
    // 18.2.2.7
    const expressions: Algebra.Expression[] = filters.map(filter => translateExpression(filter.expression));
    if (expressions.length > 0) {
      result = factory.createFilter(
        result,
        expressions.reduce((acc, exp) => factory.createOperatorExpression('&&', [ acc, exp ])),
      );
    }
...
function translateExpression(exp: Expression | RDF.Term | Wildcard): Algebra.Expression {
  ...
  if ('aggregation' in exp) {
    return factory.createAggregateExpression(
      exp.aggregation,
      translateExpression(exp.expression),
  ...
  if ('operator' in exp) {
    if (exp.operator === 'exists' || exp.operator === 'notexists') {
      return factory.createExistenceExpression(
        ...
        translateGraphPattern(<Pattern> exp.args[0]),
      );
    }
    ...
    return factory.createOperatorExpression(exp.operator, exp.args.map(<any> translateExpression));
  }
...
function translateBgp(thingy: BgpPattern): Algebra.Operation {
  ...
  for (const t of thingy.triples) {
    if ('pathType' in t.predicate) {
      // TranslatePath returns a mix of Quads and Paths
      const path = translatePath(<Triple & { predicate: PropertyPath }> t);
      for (const p of path) {
...
function translatePath(triple: Triple & { predicate: PropertyPath }): (Algebra.Path | Algebra.Pattern)[] {
  ...
  const pred = translatePathPredicate(triple.predicate);
...
function translatePathPredicate(predicate: IriTerm | PropertyPath): Algebra.PropertyPathSymbol {
  ...
  if (predicate.pathType === '!') {
    ...
    if ('pathType' in predicate.items[0] && <any> predicate.items[0].pathType === '|') {
    ...
    for (const item of items) {
      if (Util.isSimpleTerm(item)) {
        ...
        inverted.push(<RDF.NamedNode> item.items[0]);
      } else {
    ...
    const normalElement = factory.createNps(normals);
    const invertedElement = factory.createInv(factory.createNps(inverted));
...
function recurseGraph(thingy: Algebra.Operation, graph: RDF.Term, replacement?: RDF.Variable): Algebra.Operation {
  if (thingy.type === types.GRAPH) {
    ...
    for (const key of Object.keys(thingy)) {
      ...
      } else if (typeVals.includes(thingy[key].type)) {
...
function translateInlineData(values: any): Algebra.Values {
  const variables = <RDF.Variable[]>(values.values.length === 0 ?
      [] :
    Object.keys(values.values[0])).map(factory.createTerm.bind(factory));
...
function translateAggregates(query: Query, res: Algebra.Operation): Algebra.Operation {
  ...
  if (select.group ?? Object.keys(A).length > 0) {
    const aggregates = Object.keys(A).map(v =>
      translateBoundAggregate(<AggregateExpression> A[v], <RDF.Variable>factory.createTerm(v)));
    ...
    if (select.group) {
      for (const e of select.group) {
        if (Util.isSimpleTerm(e.expression)) {
          ...
          const v = 'variable' in e ? <RDF.Variable>(<any> e).variable : generateFreshVar();
  ...
  if (query.queryType === 'SELECT') {
    ...
    res = factory.createProject(res, <RDF.Variable[]> PV);
  }
...
function mapAggregate(thingy: mapAggregateType, aggregates: NodeJS.Dict<AggregateExpression>): any {
  ...
  if ('args' in thingy && thingy.args) {
    return { ...thingy, args: thingy.args.map(subthingy => mapAggregate(<mapAggregateType>subthingy, aggregates)) };
  }
...
function translateSingleUpdate(thingy: UpdateOperation): Algebra.Update {
  if ('type' in thingy) {
    ...
    return translateInsertDelete(<any> thingy);
  }

  throw new Error(`Unknown update type ${JSON.stringify(thingy)}`);
}

type TempDelIns = (BgpPattern | GraphQuads)[];
...
function translateUpdateGraph(thingy: CreateOperation | ClearDropOperation): Algebra.Update {
  ...
  switch (thingy.type) {
    ...
    case 'create': return factory.createCreate(<RDF.NamedNode> source, thingy.silent);
...
function translateUpdateGraphLoad(thingy: LoadOperation): Algebra.Load {
  return factory.createLoad(thingy.source, <RDF.NamedNode | undefined> thingy.destination, thingy.silent);
}
...
function translateBlankNodesToVariables(res: Algebra.Operation): Algebra.Operation {
  ...
  const variablesRaw: Record<string, boolean> = [ ...variables ]
